...
int main(int argc, char** argv) {
    ...
    app.add_option("--rangeKrn", rangeKrnName, "The range kernel in use (gaussian - default, gaussianUnscaled, huber, tukey, lorentz)");
    app.add_option("--kernelSize", spatialKernelSize, "The size of the spatial kernel; if not provided, it is computed based on the spatial sigma");
    ...
    CLI::Option* boxArg = app.add_flag("--box", "Use a box spatial kernel instead of a Gaussian. The size will be computed from ");
    ...
    app.add_option("--perf-sigma-num", numOfVals, "Number of values between the start and end of sigmaRange")->needs(perfArg);
    ...
    app.add_option("--perf-runs", numberOfRuns, "Number of runs per combination of sigmaRange and kernel size (does not count the first run that is discarded)")->needs(perfArg);

    CLI::Option* psnrArg = app.add_flag("--psnr",
                                        "Run an evaluation on the approximation based on PSNR"
                                        "Will run different combination of params, measure PSNR and save a CSV")->excludes(perfArg);
    ...
    app.add_option("--psnr-sigma-num", numOfVals, "Number of values between the start and end of sigmaRange")->needs(psnrArg);
